...
async function analyse(state, file) {
    var _a;
    console.log('▶ ' + file);
    ...
    await processTag(state, file, $, 'img', 'src', processImage, theFold, appendToBody);
    await processTag(state, file, $, 'iframe', 'src', processIframe, theFold, appendToBody);
    await processTag(state, file, $, 'video', 'src', processVideo, theFold, appendToBody);
    await processTag(state, file, $, 'meta[property$=":image"], meta[name$=":image"]', 'content', processMetaImage, theFold, appendToBody);
    // Remove the fold
    //
    if (theFold) {
        $('the-fold').remove();
    ...
    if (prependToHead || appendToHead) {
        if (prependToHead)
            heads.prepend(prependToHead);
        if (appendToHead)
            heads.append(appendToHead);
    }
    switch (charsetElements$.length) {
        ...
        case 1:
            ...
            if (!wasCharsetFirst || prependToHead)
...
function isElementAboveTheFold(cheerio, element, theFold) {
    const aboveTheFoldAttr = cheerio.attr(ABOVE_FOLD_DATA_ATTR);
    if (aboveTheFoldAttr) {
...
async function processImage(state, htmlfile, img, isAboveTheFold) {
    var _a, _b, _c;
    /*
     * Attribute 'src'
     */
    ...
    if (typeof fullSizeRaw !== undefined)
        img.removeAttr('data-fullsize');
    const fullSize = !!fullSizeRaw && parseInt(fullSizeRaw) > 0;
    const config = state.options;
    if (!isIncluded(attrib_src, config.image.src_include, config.image.src_exclude)) {
        // Setup to ignore
        return;
    }
    /*
    ...
    if (isExternalImage(attrib_src, external)) {
        ...
        try {
            ...
            if (external_src)
                img.attr('src', (attrib_src = external_src));
        }
        catch (e) {
    ...
    switch (config.image.cdn.process) {
        ...
        case 'optimize':
            ...
            if (cdnTransformer && !config.image.cdn.src_include) {
                throw new Error('config.image.cdn.src_include is required when specifying a config.image.cdn.transformer');
            }
            if (!cdnTransformer) {
            ...
            if (!cdnTransformer)
                break;
            if (!isIncluded(attrib_src, config.image.cdn.src_include || /.*/, config.image.cdn.src_exclude))
                break;
            if (fullSize)
                return;
            ...
            const new_srcset = await generateSrcSetForCdn(state, attrib_src, cdnTransformer, attrib_width, attrib_height);
            if (new_srcset !== null) {
     ...
     * Loading image
     */
    const originalImage = await Resource.loadResource(state, htmlfile, attrib_src);
    // No file -> give up
    if (!originalImage) {
        state.reportIssue(htmlfile, {
            type: 'erro',
    ...
    if ((newImage === null || newImage === void 0 ? void 0 : newImage.data) && newImage.data.length < (await originalImage.getLen())) {
        const newFilename = path.join(path.dirname(originalImage.filePathAbsolute), path.basename(originalImage.filePathAbsolute, path.extname(originalImage.filePathAbsolute)) + `.${newImage.format}`);
        const newSrc = path.join(path.dirname(attrib_src), path.basename(attrib_src, path.extname(attrib_src)) +
            `.${newImage.format}`);
        if (!state.compressedFiles.has(newFilename) && !state.args.nowrite) {
            await ((_b = state.vfs) !== null && _b !== void 0 ? _b : fsp).writeFile(newFilename, newImage.data);
        }
    ...
    if (img.attr('srcset')) {
        ...
        const new_srcset = await generateSrcSet(state, htmlfile, originalImage, img.attr('src'), imageLengthToBeatInSrcSet, { toFormat: srcToFormat });
        if (new_srcset !== null) {
            img.attr('srcset', new_srcset);
        }
        // Add sizes attribute if not specified
    ...
    if (isInPicture) {
        ...
        switch (mime) {
            ...
            case 'image/webp':
                ...
                const isLossless = mime === 'image/png' ||
                    (mime === 'image/webp' &&
                        ((_c = (await originalImage.getImageMeta())) === null || _c === void 0 ? void 0 : _c.isLossless));
                if (!isLossless) {
                    sourcesToGenerate.push({
                        mime: 'image/avif',
                        format: 'avif',
        ...
        for (const s of sourcesToGenerate.reverse()) {
            ...
            const srcset = await generateSrcSet(state, htmlfile, originalImage, undefined, undefined, {
                toFormat: s.format,
            });
            if (!srcset) {
...
async function processMetaImage(state, htmlfile, meta) {
    var _a, _b, _c;
    ...
    if (!isIncluded(content, src_include, src_exclude))
        return;
    /*
     * Check for external images
     */
    if (isExternalImage(content, external)) {
        ...
        try {
            ...
            if (external_content)
                meta.attr(attribute, (content = external_content));
        }
        catch (e) {
            return state.reportIssue(htmlfile, {
                type: 'warn',
    ...
    switch (cdn.process) {
        ...
        case 'optimize':
            ...
            if (!cdnTransformer)
                break;
            if (!isIncluded(content, cdn.src_include || /.*/, cdn.src_exclude))
                break;
            if (!width)
                return state.reportIssue(htmlfile, {
                    type: 'warn',
            ...
            meta.attr(attribute, (_b = (_a = cdnTransformer({ url: content, width: +width })) === null || _a === void 0 ? void 0 : _a.toString()) === null || _b === void 0 ? void 0 : _b.replace(UNPIC_DEFAULT_HOST_REGEX, '/'));
            return;
    }
    /*
     * Loading image
    ...
    const newImage = await compressImage(state, await originalImage.getData(), {
        ...
        toFormat: meta.closest(`meta[${propName}:type]`).attr(attribute) === 'image/png'
            ? 'png'
            : 'jpeg',
    });
    if ((newImage === null || newImage === void 0 ? void 0 : newImage.data) && newImage.data.length < (await originalImage.getLen())) {
        meta.attr(attribute, path.join(path.dirname(content), path.basename(content, path.extname(content)) + `.${newImage.format}`));
        const newFilename = path.join(path.dirname(originalImage.filePathAbsolute), path.basename(originalImage.filePathAbsolute, path.extname(originalImage.filePathAbsolute)) + `.${newImage.format}`);
        if (!state.compressedFiles.has(newFilename)) {
            if (!state.args.nowrite)
                await ((_c = state.vfs) !== null && _c !== void 0 ? _c : fsp).writeFile(newFilename, newImage.data);
            // Mark new file as compressed
            state.compressedFiles.add(newFilename);
            // Report compression result
            state.reportSummary({
...
const isIncluded = (src, includeConf, excludeConf) => !!src.match(includeConf) && (!excludeConf || !src.match(excludeConf));
async function _generateSrcSet({ options }, startSrc, imageWidth, imageHeight, transformSrc) {
...
async function generateSrcSet(state, htmlfile, originalImage, startSrc, startSrcLength, options) {
    ...
    return _generateSrcSet(state, startSrc, imageWidth, imageHeight, async (valueW) => {
        var _a;
        ...
        if (!state.compressedFiles.has(absoluteFilename)) {
            const compressedImage = await compressImage(state, await originalImage.getData(), { ...options, resize: { width: valueW } });
            if (!(compressedImage === null || compressedImage === void 0 ? void 0 : compressedImage.data) ||
                ...
                // Write file
                if (!state.args.nowrite) {
                    await ((_a = state.vfs) !== null && _a !== void 0 ? _a : fsp).writeFile(absoluteFilename, compressedImage.data);
                }
...
async function generateSrcSetForCdn(state, startSrc, cdnTransformer, imageWidth, imageHeight) {
    return _generateSrcSet(state, '', imageWidth, imageHeight, (valueW) => {
        var _a, _b;
        return (_b = (_a = cdnTransformer({
            url: startSrc,
            width: valueW,
        })) === null || _a === void 0 ? void 0 : _a.toString()) === null || _b === void 0 ? void 0 : _b.replace(UNPIC_DEFAULT_HOST_REGEX, '/');
    });
}
async function setImageSize(state, htmlfile, img, image) {
...
export async function optimize(state, include, exclude) {
    var _a;
    ...
    for (const file of paths) {
        ...
        (_a = state.onAnalysedFile) === null || _a === void 0 ? void 0 : _a.call(state, file);
    }
}
async function processTag(state, file, $, tag, attribute_to_log, processor, theFold, appendToBody) {
    ...
    for (let i = 0; i < tagsArray.length; i++) {
        ...
        spinnerImg.text = kleur.dim(`<${tag}> [${i + 1}/${tagsArray.length}] ${$(tagElement).attr(attribute_to_log)}`);
        const el = $(tagElement);
    ...
    // Reset spinner
    spinnerImg.text = kleur.dim(`<${tag}> [${tagsArray.length}/${tagsArray.length}]`);
    // Notify issues
    const issues = state.issues.get(file);
    const newIssues = ((issues === null || issues === void 0 ? void 0 : issues.length) || 0) - ((previous_issues === null || previous_issues === void 0 ? void 0 : previous_issues.length) || 0);
    if (newIssues > 0) {
        spinnerImg.fail();
    }
    else {
...
const isExternalImage = (content, external) => !isLocal(content) &&
    isIncluded(content, external.src_include, external.src_exclude);
async function externalImage(content, state, htmlfile) {
